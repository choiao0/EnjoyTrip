      ...
      <div class="row g-4">
        ...
        <div class="col-lg-7">
          ...
          <section class="bg-white border rounded-3 p-4 mb-4">
            ...
            <div v-if="loading" class="text-muted py-5 text-center">AI가 답변을 생성하고 있습니다...</div>
            <div v-else-if="error" class="alert alert-danger mb-0">{{ error }}</div>
            <div v-else-if="answer" style="white-space:pre-wrap;">{{ answer }}</div>
            <div v-else class="text-muted py-5 text-center">질문을 입력하고 버튼을 눌러보세요.</div>
          </section>

          <!-- 관광지 카드 -->
          <div v-if="attractionCards.length" class="d-flex flex-column gap-3">
            <div
              v-for="a in attractionCards"
              :key="a.contentId"
